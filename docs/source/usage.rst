...
One of three databases (`DRAMP <http://dramp.cpu-bioinfor.org/>`_, `APD <https://aps.unmc.edu/>`_, or `UniRef100 <https://academic.oup.com/bioinformatics/article/23/10/1282/197795>`_) can be chosen via the ``--amp_database`` parameter, with `DRAMP <http://dramp.cpu-bioinfor.org/>`_ set as the default. 
...
To get a full list of available options and their defaults please refer to the help documentation of the ``parse_tables`` submodule:
